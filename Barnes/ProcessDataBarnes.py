...
class ProcessDataBarnes:
	...
	def generate_staging_output(self, logger, filename, agg_rules, default_config, extracted_data):
		...
		if extracted_data['disc_percentage'].dtype == 'object':
			extracted_data['disc_percentage'] = pd.to_numeric(extracted_data['disc_percentage'].str.rstrip('%'), errors='coerce')/100
		...
		extracted_data['sale_type'] = extracted_data.apply(lambda row: ('REFUNDS') if(row['sale_type'] == 'CRE') else ('PURCHASE'), axis=1)
		...
		for each_file in files_in_s3:
			if each_file != '' and each_file.split('.')[-1] != 'txt':
				...
				try:
					...
					if not data.empty:
						logger.info('Get the corresponding rules object for Barnes')
						agg_rules = next((item for item in rule_config if (item['name'] == 'BARNES')), None)
