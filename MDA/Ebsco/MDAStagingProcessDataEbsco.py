...
class MDAStagingProcessDataEbsco :
    ...
    def generate_edw_staging_data(self,logger, agg_rules, default_config, app_config,final_mapped_data) :
        ...
        final_mapped_data['units'] = final_mapped_data['units'].astype('float')
        ...
        final_mapped_data['sale_type'] = final_mapped_data.apply(
            lambda row : ('REFUNDS') if(row['sale_type'] == 'NA' and (row['units'] < 0 or row['payment_amount'] < 0)) else row[
                'sale_type'],
            axis=1)
        ...
        final_mapped_data['list_discount_percentage'] = final_mapped_data.apply(
            lambda row : ('0') if row['list_discount_percentage'] == 'NA' else row['list_discount_percentage'], axis=1)
        ...
        final_mapped_data['current_discount_percentage'] = final_mapped_data['current_discount_percentage'].astype(
            'str')
        final_mapped_data['current_discount_percentage'] = (
        final_mapped_data['current_discount_percentage']).str.rstrip()
        final_mapped_data['current_discount_percentage'] = pd.to_numeric(
            final_mapped_data['current_discount_percentage'])
        ...
        final_mapped_data['list_discount_percentage'] = (final_mapped_data['list_discount_percentage']).replace('%', '',
                                                                                                                regex=True)
        final_mapped_data['list_discount_percentage'] = final_mapped_data['list_discount_percentage'].astype('str')
        final_mapped_data['list_discount_percentage'] = (final_mapped_data['list_discount_percentage']).str.rstrip()
        final_mapped_data['list_discount_percentage'] = pd.to_numeric(final_mapped_data['list_discount_percentage'])

        final_mapped_data['list_discount_percentage'] = final_mapped_data.apply(
            lambda row : row['list_discount_percentage'] * 100 if (row['list_discount_percentage'] < 1) else row[
                'list_discount_percentage'],
            axis=1)
