...
class MDAMappedProcessDataGardners:
	...
	def process_trans_type(self, logger, extracted_data):
		...
		extracted_data['sale_type'] = extracted_data.apply(lambda row: ('Return') if(row['sale_type_ori'] == 'CREDIT') else ('Purchase'), axis=1)
		extracted_data['trans_type'] = extracted_data.apply(lambda row: ('Rental') if(row['trans_type_ori'] == 'RENTAL') else 'Sales', axis=1)
