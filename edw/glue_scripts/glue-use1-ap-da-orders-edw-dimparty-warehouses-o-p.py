...
def create_view_tandf_customer(mda_table):
    ...
    '''    
    db_sql = f"""
        SELECT billing_customer_id from mda_data_lake.{mda_table} 
            WHERE billing_customer_id != 'NA' or billing_customer_id is not null
        """
    
    if aggregator_name != 'AUSTLD':
        db_sql += f"""
            UNION
            SELECT shipping_customer_id from mda_data_lake.{mda_table} 
                WHERE shipping_customer_id != 'NA' or shipping_customer_id is not null
            """
    
    spark.sql("use mda_data_lake")
    print("db_sql_0: ", db_sql)
...
def fetch_non_match_customer_id():
    ...
    leftjoin_df = spark.sql("""
        SELECT a.*, b.* from tandf_eq_tab a left join (select * from dimparty_tab where party_type in ('Warehouses', 'P-Aggregators', 'E-Aggregators', 'Owned Sites', 'Customer') and external_id != '') b on a.billing_customer_id = b.external_id"""
        ).cache()

    non_match_cid = leftjoin_df.filter("external_id is null and billing_customer_id is not null")
...
def update_party_skey(staging_table, connection_options):
    ...
    DDFrameForGlue_df1 = DynamicFrame.fromDF(temp_df, glueContext, "DDFrameForGlue_df1")
    resolvechoice11 = ResolveChoice.apply(frame=DDFrameForGlue_df1, choice="make_cols", transformation_ctx="resolvechoice11")
        
    preactions = f"""update {staging_table} set seller_party_skey=s.party_skey from edw.dim_party s where billing_customer_id=s.external_id;"""
    preactions += f"""update {staging_table} set fulfilment_party_skey=s.party_skey from edw.dim_party s where source=s.party_name;"""

    if aggregator_name != 'AUSTLD':
        preactions += f"""update {staging_table} set buyer_party_skey=s.party_skey from edw.dim_party s where shipping_customer_id=s.external_id;"""
        
    print("Post Query: ", preactions)
    print(f"< insertion started for {staging_table}>")
        
    datasink11 = glueContext.write_dynamic_frame.from_jdbc_conf(
        frame=resolvechoice11, catalog_connection=db_variables['db_connection'], 
        connection_options={"preactions":preactions, "dbtable":staging_table, "database":db_variables['database']},
...
def initialise():
    ...
    create_view_dimparty(connection_options)
