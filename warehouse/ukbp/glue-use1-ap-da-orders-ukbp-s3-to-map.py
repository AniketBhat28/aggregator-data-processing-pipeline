...
def read_data(time_frame):
    ...
    :return: None
    '''
    datasource0 = spark.read.option("header", "true").csv(
        's3://' + input_bucket_name + '/' + input_dir_path + '/' + time_frame + '*/*sales*'
        )
    print("> datasource0 count > ", datasource0.count())

    df1= datasource0.withColumn("input_filename",input_file_name())
    df1 = df1.withColumn('input_filename', regexp_replace('input_filename', 's3://' + input_bucket_name + '/' + input_dir_path + '/', ''))
...
def save_parquet(time_frame, year, output_dir_path):
    ...
    from
        (SELECT
            ...
            'UKBP' as source, input_filename as source_id, SA_PACK_FLAG as sa_pack_flag 
        from
            salesdata
        )t""")
